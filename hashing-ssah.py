...
class Hashing(object):
    ...
    def set_lr(self):
        ...
        self.lr_lab = [5e-4] * 100
        self.lr_img = [5e-4] * 100
        self.lr_txt = [5e-4] * 100
        self.lr_dis = [5e-4] * 100
    ...
    def train_lab_net(self):
        ...
        for (_, _, label, idx) in tqdm(self.train_loader):
            ...
            loss = self.model.semantic_loss(lab_code, lab_feat, lab_pred, label)
    ...
    def train_img_net(self):
        ...
        for (img, _, label, idx) in tqdm(self.train_loader):
            ...
            img_code, img_feat, img_pred = self.model.img_forward(img, idx)

            loss = self.model.semantic_loss(img_code, img_feat, img_pred, label) \
    ...
    def train_txt_net(self):
        ...
        for (_, txt, label, idx) in tqdm(self.train_loader):
            ...
            loss = self.model.semantic_loss(txt_code, txt_feat, txt_pred, label) \
...
def parser_arguments():
    ...
    parser.add_argument('--epochs', dest='epochs', type=int, default=100, help='number of epoch')
    parser.add_argument('--K_lab', dest='K_lab', type=int, default=2, help='training iterations in each epoch')
    parser.add_argument('--K_img', dest='K_img', type=int, default=2, help='training iterations in each epoch')
    parser.add_argument('--K_txt', dest='K_txt', type=int, default=2, help='training iterations in each epoch')
